...
'''
python train.py 
-i path-to-hdf5-train-file/ 
-u number-of-hidden-units 
-d number-of-delay-frames 
-c number-of-context-frames 
-o output-folder-to-save-model-file
'''
...
def dataGenerator():
    ...
    idxList = [i for i in range(dset['flmark'].shape[0])]
...
class AirModel(nn.Module):
    def __init__(self):
        super().__init__()
        self.lstm_1 = nn.LSTM(input_size=512, hidden_size=512, num_layers=4, batch_first=True)
        ...
        # self.lstm_4 = nn.LSTM(input_size=512, hidden_size=136, batch_first=True)
...
class Custom_Dataset(Dataset):
    ...
    def __getitem__(self,idx):
        ...
        X = self.addContext(cur_mel, ctxWin)
        Y_= cur_lmark

        out = {'X':torch.from_numpy(X), 'Y':torch.from_numpy(Y_)}
        return out 
...
for epoch in tqdm(range(numEpochs)):
    for i in tqdm(range(numIt)):
        ...
        # X_test.shape : (128,75,512)
        # Y_test.shape : (128,75,136)
